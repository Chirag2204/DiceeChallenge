...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      // Create
      profile = new serviceprovider(profileFields);
...
router.get('/', async (req, res) => {
  try {
    ...
    const profiles = await serviceprovider.find({...keyword}).populate('user', ['name', 'avatar']);
    res.json(profiles);
  } catch (err) {
    console.error(err.message);
    res.status(500).send('Server Error');
...
// @access   Private
router.post('/user/:user_id/reviews', protect, async (req, res) => {
  try {
...
router.delete('/', protect, async (req, res) => {
  try {
    // Remove profile
    await serviceprovider.findOneAndRemove({ user: req.user.name });
    // Remove user
...
router.delete('/education/:edu_id', protect, async (req, res) => {
  try {
    const foundProfile = await serviceprovider.findOne({ user: req.user.id });
